...
- **메타 블록** — 페이지 맨 위 콜아웃에 `제목:` `카테고리:`(`Security` | `Engineering` | `Insight`) `요약:`(1~2문장) `저자:`(이름 · 역할)를 적는다. 선택: `썸네일:`("첫 번째 이미지" 또는 파일명), `날짜:`, `slug:`. 노션 API가 페이지 제목을 돌려주지 않아 **제목도 여기에 적어야 한다.** 이 블록은 본문으로 옮기지 않는다.
- **본문** — 노션 기본 블록만 쓴다: 제목2/제목3, 문단, 코드, 이미지, 인용, 콜아웃, 글머리 기호/번호 목록, 표. 아래 대응표에 없는 블록(토글, 데이터베이스, 임베드, 북마크, 컬럼, 동기화 블록, 수식, 페이지 멘션, 파일)은 변환되지 않는다.
- 저자 관점의 상세 안내(서식·이미지·구조 팁·프리뷰 확인)는 `blog-author-guide.md`에 있다.
...
| 굵게/기울임/형광펜 | `<strong>` / `<em>` / `<em>` (em은 상세 페이지가 기울임 대신 색·굵기 강조로 렌더 — 한글 faux italic 겹침 방지). 글자색·밑줄은 무시 | — (HTML 그대로)                  |
...
- 원문에 마크다운 별표가 글자로 남은 경우(`\*\*텍스트\*\*`)는 의도가 명백하므로 `<strong>`으로 옮기고 결과 보고에 적는다. 그 외 오타·표기는 각색하지 않는다.
- 같은 slug가 이미 등록돼 있으면 수정으로 취급한다 — 본문 컴포넌트를 덮어쓰고, 메타는 바뀐 항목만 갱신, slug·날짜 유지.
