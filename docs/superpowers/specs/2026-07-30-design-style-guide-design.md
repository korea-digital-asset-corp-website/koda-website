...
| 6 | 컨테이너 클래스 문자열이 섹션마다 복붙 | 문자열을 **`max-w-[1440px] w-full px-5 lg:px-10 mx-auto`로 고정**하고 신규 코드도 이 문자열을 그대로 쓴다. 공용 컴포넌트 추출은 비목표다. |
...
2. **유틸 생성 확인** — 3.3-1에서 "유틸 클래스를 쓴다"를 표준으로 정했으나, 알파 변형 토큰(`--color-gray-5020`, `--color-gray-5050`)이 실제로 `bg-gray-5020` 형태의 유틸을 만드는지는 확인되지 않았다. 기존 코드는 전부 `bg-[var(--color-gray-5020)]`로 우회하고 있다. 가이드에 이 클래스를 적기 전에 실제 렌더링으로 확인한다. 유틸이 생성되지 않으면 해당 두 토큰만 예외로 두고 `bg-[var(--color-gray-5020)]` 형태를 표준으로 명시한다.
