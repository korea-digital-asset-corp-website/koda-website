...
import { H2, H3, H4, H5, Body2Pre } from "src/components/Typography";
...
• 보유 기간: 3개월 이상`}</Description>
          ...
          <Section>
            ...
            <DescriptionDivider />
            <DescriptionTitle>{`개인정보처리 수탁업체(국내)`}</DescriptionTitle>
            <Divider16 />
            <DescriptionBold>{`(주) 해치랩스`}</DescriptionBold>
            <Description>{`• 위탁 내용: 수탁시스템 유지보수`}</Description>
            <Divider32 />
...
• 위탁 항목: 이름, 이메일, 서비스 이용 기록, 접속 로그, 접속 IP 정보
...
⑤ 회사는 이용자 권리에 따른 열람의 요구, 정정·삭제의 요구, 처리정지의 요구 시 열람 등 요구를 한 자가 본인이거나 정당한 대리인인지를 확인합니다.`}</Description>
          ...
          <Section>
            ...
            <Description>{`• 필수항목 : 이메일, 휴대전화번호, 주소, 생년월일, 이름, 국적, 직책, 회사명, 서비스 이용 기록, 접속 로그, 접속 IP 정보`}</Description>
          </Section>
          <Section>
...
• 경찰청 사이버수사국 : (국번없이) 182 (cyberbureau.police.go.kr)`}</Description>
          </Section>
          <Section>
          ...
          <Section>
            ...
            <Description>{`(시행일) 이 개인정보처리방침은 2021년 1월 1일부터 적용됩니다. 
*변경 내역: [특정 금융거래정보의 보고 및 이용에 관한 법률 시행령]에 의거 트래블룰 적용에 따른 자산이동 수집항목 추가`}</Description>
          </Section>
          <FooterView />
        </Content>
      </Container>
